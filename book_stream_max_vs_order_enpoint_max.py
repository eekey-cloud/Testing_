#!/usr/bin/env python3
"""
Book-stream vs GET /order max size, plotted over time.

    pip install websocket-client requests matplotlib
    python book_order_v5.py
...
v5: the size grid now spans 0.01x to 3.0x, so when the book max drops the
    test still has resolution below it. The old 1.0x-2.0x grid could only
    report ">= 1.0x" or "nothing", which made every book dip look like an outage.
"""
...
MINT = "86uRNkDCXgM4PASTE_THE_FULL_MINT_HERE"
# ===========================================================================
        ...
        try:
            ...
            while not stop.is_set():
                ...
                for b in frame.get("updates") or []:
                    ...
                    with lock:
                        latest.update(ts=ts, buy_max=ac, sell_max=bt_)
...
def plot(bpath, opath, base):
    ...
    series = {}
    for side, col in (("buy", "max_cash"), ("sell", "max_tokens")):
        tag = "ask" if side == "buy" else "bid"
        bx = [dt(r["ts"]) for r in bk if r["side"] == tag and float(r["ts"]) >= t_from]
        by = [float(r[col]) for r in bk if r["side"] == tag and float(r["ts"]) >= t_from]
        ox, oy = [], []
        for rid in sorted({r["round_id"] for r in od}, key=int):
            ...
            if not g or float(g[0]["round_ts"]) < t_from:
                continue
            f = [float(r["size_human"]) for r in g if r["ok"] == "True"]
    ...
    for ax, side, unit in ((axes[0], "buy", "CASH in"), (axes[1], "sell", "tokens in")):
        ...
        ax.set_ylabel(f"{side.upper()}  ({unit})", fontsize=13, fontweight="bold")
...
def text_summary(bpath, opath):
    ...
    for r in bk:
        book["buy" if r["side"] == "ask" else "sell"].append(
            (float(r["ts"]), float(r["max_cash"] if r["side"] == "ask" else r["max_tokens"])))
    for k in book:
        book[k].sort()
    ...
    for side in ("buy", "sell"):
        ...
        for rid in sorted({r["round_id"] for r in od}, key=int):
            ...
            if not g:
                continue
            f = [(float(r["mult"]), float(r["size_human"])) for r in g if r["ok"] == "True"]
            if not f:
                dead += 1
                continue
            if max(m for m, _ in f) >= max(MULTIPLIERS) - 1e-9:
                ceil += 1
            bm = book_at(side, float(g[0]["round_ts"]))
            if bm:
...
def main():
    ...
    t_end, samples = time.time() + CALIBRATE_SEC, {"buy": [], "sell": []}
    ...
    sizes, base = {}, {}
    for side in ("buy", "sell"):
        s = sorted(samples[side])
        base[side] = s[len(s) // 2]
        sizes[side] = [(m, base[side] * m) for m in MULTIPLIERS]
        print(f"    {side:<5} book max over 10s: min {s[0]:,.2f}  "
              f"median {base[side]:,.2f}  max {s[-1]:,.2f}")
